...
g_loss_gan = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_fake,
labels=y_real))
g_loss_L1 = L1_lambda*tf.reduce_mean(tf.squared_difference(x_fakeB, x_realB))

g_loss = g_loss_gan + g_loss_L1

d_loss_real = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_real,
labels=y_real))
d_loss_fake = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=d_fake,
labels=y_fake))
